...
from helpers import getLenght, length, ray_segment_intersect
...
def rayTrace():
    for i in range(0, 3, 1):
        ...
        if isinstance(closest, Point):
            pygame.draw.line(
                window, Color, (sources[0].x, sources[0].y), (closest.x, closest.y), 2)
...
def render():
    ...
    while True:
        ...
        image_point = Point(random.uniform(0, 500), random.uniform(0, 500))
        pixel_color = 0
        for light_source in light_sources:
            ...
            for wall in segments:
                ...
                distancia_interseccion = ray_segment_intersect(
                    image_point, direccion, wall.point1, wall.point2)
                if distancia_interseccion > 0:
                    free = False
                    break
